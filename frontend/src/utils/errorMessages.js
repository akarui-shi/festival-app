const MESSAGE_MAP = [
  ...
  { match: ['пользователь с таким логином уже существует'], message: 'Этот логин уже используется.' },
  { match: ['электронная почта уже используется', 'email already exists'], message: 'Эта электронная почта уже используется.' },
  { match: ['пользователь с таким email уже существует'], message: 'Эта электронная почта уже используется.' },
  { match: ['телефон уже используется', 'пользователь с таким номером телефона уже существует'], message: 'Этот номер телефона уже используется.' },
  ...
  { match: ['нельзя сохранить публикацию: связанное мероприятие не найдено'], message: 'Выбранное мероприятие не найдено. Обновите список и попробуйте снова.' },
  { match: ['нельзя сохранить публикацию: автор не найден'], message: 'Не удалось определить автора публикации. Повторите вход в систему.' },
  { match: ['publications_status_check'], message: 'Ошибка статуса публикации. Обновите страницу и повторите попытку.' },
  { match: ['недопустимый переход статуса публикации'], message: 'Невозможно выполнить это действие для текущего статуса публикации.' },
  { match: ['статус публикации уже установлен'], message: 'Публикация уже находится в этом статусе.' },
  ...
  { match: ['нарушение целостности данных', 'data integrity violation'], message: 'Операция не выполнена: есть связанные данные.' },
  { match: ['could not execute statement'], message: 'Операция не выполнена: есть связанные данные.' },
