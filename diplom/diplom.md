...
> Рисунок 2.2 — Даталогическая модель данных (полная ERD)
>
> *Инструкция по построению:* открыть pgAdmin 4 → выбрать базу данных `festival_db` → Tools → ERD Tool → Add Table → выбрать схему `public` → добавить все таблицы. pgAdmin автоматически отобразит связи по внешним ключам. Для компактного представления: View → Auto Layout → Hierarchical. Экспорт: File → Save As Image (PNG). Альтернатива: DBeaver → Database → New ERD Diagram → Export → PNG.

На рисунке 2.3 представлена диаграмма ключевых таблиц и связей в нотации Crow's Foot, охватывающая основные сущности предметной области:
...
*Рисунок 2.3 — Диаграмма ключевых таблиц и связей даталогической модели (Crow's Foot)*

#### 2.1.3 Описание ключевых таблиц и проектных решений

Базовая схема базы данных содержит **30 таблиц** и реализована на PostgreSQL 16. В ходе последующих миграций (V3–V8) добавлено ещё **5 таблиц**, итоговое количество составляет **35 таблиц**. Управление эволюцией схемы осуществляется с помощью Flyway: восемь пронумерованных SQL-скриптов фиксируют полную историю изменений структуры. Перечень таблиц представлен в таблице 2.1.
...
**Управление эволюцией схемы через Flyway.** Все изменения схемы фиксируются как пронумерованные SQL-скрипты и хранятся в директории `src/main/resources/db/migration`:

- **V1** — создание полной базовой схемы из 30 таблиц;
- **V2** — наполнение справочников начальными данными (seed data);
- **V3** — добавление таблицы `user_interests` для хранения интересов пользователей;
- **V4** — создание составного индекса полнотекстового поиска по мероприятиям;
- **V5** — добавление таблицы `session_waitlist` для управления списком ожидания;
- **V6** — добавление таблицы `notifications` для внутренних уведомлений пользователей;
- **V7** — добавление таблицы `promo_codes` для реализации системы промокодов;
- **V8** — добавление таблицы `organization_follows` для подписок на организации.

Применение Flyway обеспечивает воспроизводимость развёртывания и защищает от несогласованных ручных изменений схемы.
...
Проект организован по принципу многоуровневой (layered) архитектуры, при которой каждый слой имеет чётко определённую ответственность и взаимодействует только со смежными слоями [11]. Корневой пакет приложения — `com.festivalapp.backend` — содержит следующие подпакеты.

> Рисунок 2.5 — Структура пакетов серверной части проекта
>
> *Рекомендации по построению:* скриншот дерева директорий проекта в IDE (IntelliJ IDEA) с раскрытым пакетом `com.festivalapp.backend`.
...
На рисунке 2.6 представлена диаграмма компонентов серверной части, отражающая взаимодействие между слоями:
...
*Рисунок 2.8 — Структура REST API приложения (таблица 2.2)*
...
Управление схемой базы данных осуществляется с помощью инструмента **Flyway** [40]. Скрипты миграций хранятся в директории `src/main/resources/db/migration` и именуются в соответствии с соглашением `V{номер}__{описание}.sql`. При каждом запуске приложения Flyway автоматически применяет ещё не выполненные миграции в порядке возрастания номеров версий, фиксируя применённые скрипты в служебной таблице `flyway_schema_history`.

Перечень миграций проекта с кратким описанием приведён в таблице 2.1 (строки 1 и 31–35). Данный подход обеспечивает воспроизводимость развёртывания в любой среде и защищает от случайных ручных изменений схемы.

#### 2.2.7 Интеграция с внешними сервисами

На рисунке 2.11 представлена C4-диаграмма контейнеров системы, отражающая взаимодействие с внешними сервисами.

> Рисунок 2.11 — C4-диаграмма контейнеров системы
>
> *Инструкция по построению:* выполнить в diagrams.net (draw.io) или Structurizr. Контейнеры: Browser (React SPA), Backend (Spring Boot), Database (PostgreSQL), File Storage (папка `uploads/`). Внешние системы: YooKassa, SMTP (Yandex Mail), Google OAuth2, Яндекс OAuth2, Яндекс Карты JavaScript API. Связи: Browser → Backend (HTTPS/REST + JWT); Backend → Database (JDBC/JPA); Backend → YooKassa (HTTPS, Webhook); Backend → SMTP (SSL/465); Browser → Яндекс Карты (JS API).
...
**Развёртывание посредством Docker Compose.** Приложение контейнеризировано с помощью Docker; конфигурация Docker Compose объединяет три сервиса:
...
Данная конфигурация обеспечивает воспроизводимое развёртывание: единственная команда `docker compose up` поднимает все три контейнера с автоматическим применением миграций Flyway [41].
...
Приложение реализует три независимых пользовательских пространства с разными точками входа и навигацией. На рисунке 2.13 представлена общая схема навигации:
...
На рисунке 2.21 представлена архитектурная диаграмма клиентской части приложения:
...
На рисунке 2.23 представлена диаграмма компонентов страницы `EventDetailPage`:
...
В разделе 2.1 спроектирована база данных, включающая 35 таблиц. Инфологическая модель формализует предметную область через сущности и связи; принципиальным решением является объединение отзывов и комментариев в единую таблицу `comments`. Даталогическая модель реализована на СУБД PostgreSQL 16 с применением паттерна Soft Delete и механизма миграций Flyway (8 версий). Разработанная структура обеспечивает целостность данных через декларативные ограничения внешних ключей и поддерживает полный жизненный цикл мероприятия — от создания черновика до архивирования.

В разделе 2.2 разработана серверная часть на платформе Java / Spring Boot 3. Реализована многоуровневая архитектура с разделением на слои контроллеров, сервисов и репозиториев. REST API охватывает все функциональные требования системы: управление мероприятиями с многокритериальной фильтрацией, билетную систему с интеграцией платёжного шлюза YooKassa, систему безопасности на основе JWT с поддержкой OAuth2 через Яндекс и Google, email-уведомления и аналитику посещаемости.
