...
На основе анализа требований (раздел 1.5) были выделены следующие ключевые сущности: Пользователь, Роль, Организация, Мероприятие, Сеанс, Площадка, Город, Артист, Категория, Тип билета, Заказ, Позиция заказа, Билет, Платёж, Возврат, Комментарий/отзыв, Публикация, Изображение, Уведомление, Промокод.
...
На рисунке 2.4 представлена диаграмма последовательности процесса оформления и оплаты заказа на билет:
...
Для описания архитектуры системы применяется нотация C4 Model, предложенная Саймоном Брауном [46]. Модель C4 организует архитектурное представление системы в виде четырёх последовательно детализируемых уровней: Контекст (System Context), Контейнеры (Containers), Компоненты (Components) и Код (Code).
...
Документация API сформирована с помощью SpringDoc OpenAPI (Swagger UI) и доступна по адресу `/swagger-ui.html` в ходе разработки.
...
**Развёртывание посредством Docker Compose.** Приложение контейнеризировано с помощью Docker; конфигурация Docker Compose объединяет три сервиса: PostgreSQL (`db`), Spring Boot Backend (`backend`) и React-фронтенд (`frontend`). Единственная команда `docker compose up` поднимает все сервисы с автоматическим применением миграций Flyway при старте бэкенда [45].
...
46. Brown S. The C4 model for visualising software architecture [Электронный ресурс]. — URL: https://c4model.com/ (дата обращения: 15.03.2026).
