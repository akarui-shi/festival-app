...
На основе анализа требований (раздел 1.5) были выделены следующие ключевые сущности: Пользователь, Роль, Организация, Мероприятие, Сеанс, Площадка, Город, Артист, Категория, Тип билета, Заказ, Позиция заказа, Билет, Платёж, Возврат, Комментарий, Избранное, Публикация/новость, Изображение. Состав сущностей и связи между ними отражены на рисунке 2.1.

Рисунок 2.1 — Инфологическая ER-диаграмма системы
Инфологическая модель отражает следующие ключевые связи между сущностями:
1)	Пользователь может иметь одну или несколько Ролей (M:N через user_roles);
2)	Пользователь состоит в Организации с определённым статусом участия (M:N через organization_members);
3)	Организация организует Мероприятия (1:M);
4)	Мероприятие содержит один или несколько Сеансов (1:M); каждый сеанс проводится на Площадке (M:1) и может иметь координаты, переопределяющие местоположение площадки;
5)	Сеанс имеет набор Типов билетов (1:M), на основе которых формируются Позиции заказа (M:N), объединяемые в Заказы; каждый заказ связан с одним Платежом (1:1), платёж может иметь Возврат (1:0..1);
6)	Пользователь оставляет Комментарии/отзывы к мероприятиям (1:M) и добавляет мероприятия в Избранное (M:N);
7)	Мероприятие включает Артистов (M:N через event_artists), относится к Категориям (M:N через event_categories) и содержит Публикации-новости (1:M).
...
Временные метки. Для всех временных полей используется тип `timestamp with time zone` (PostgreSQL) и `OffsetDateTime` (Java), что обеспечивает корректную работу с часовыми поясами при международном использовании и сохранении логов административных действий с учётом исходной зоны.

Индексирование. Помимо первичных ключей и автоматических индексов на внешних ключах, в таблицах с полнотекстовым поиском (`events`, `publications`) созданы индексы типа GIN над выражениями `tsvector` (миграция V4), а в таблице `tickets` — уникальный индекс на колонку `qr_token`, гарантирующий уникальность QR-кода на уровне СУБД и быстрый поиск при верификации билета.
...
Проект организован по принципу многоуровневой (layered) архитектуры, при которой каждый слой имеет чётко определённую ответственность и взаимодействует только со смежными слоями [32]. Такой подход обеспечивает разделение ответственности (Separation of Concerns), изолирует бизнес-логику от деталей хранения данных и транспортного протокола, упрощает модульное тестирование и сопровождение системы. Корневой пакет приложения — `com.festivalapp.backend` — содержит следующие подпакеты: `entity`, `repository`, `service`, `controller`, `dto`, `security`, `config`, `exception`.

Каждый пакет выполняет строго определённую роль:
1) `entity` — JPA-сущности (43 класса), отображаемые на таблицы базы данных PostgreSQL посредством ORM Hibernate;
2) `repository` — интерфейсы Spring Data JPA для доступа к данным; за счёт автоматической генерации реализаций исключают необходимость написания шаблонного кода CRUD-операций;
3) `service` — бизнес-логика (33 сервиса): `EventService`, `OrderService`, `PaymentGatewayService`, `WaitlistService`, `PromoCodeService`, `NotificationService` и др.; границы транзакций задаются аннотацией `@Transactional`;
4) `controller` — REST-контроллеры (23 контроллера), обрабатывающие входящие HTTP-запросы и делегирующие их в сервисный слой;
5) `dto` — объекты передачи данных (более 70 классов), изолирующие внутреннее представление сущностей от внешнего API и предотвращающие утечку чувствительных полей;
6) `security` — компоненты аутентификации и авторизации (`JwtAuthFilter`, `JwtService`, `CustomUserDetailsService`, OAuth2-обработчики);
7) `config` — конфигурационные классы Spring (`SecurityConfig`, `WebMvcConfig`, `CorsConfig`, `YandexConfig`);
8) `exception` — пользовательские исключения и единый обработчик ошибок (`@RestControllerAdvice`), формирующий унифицированный JSON-ответ с кодом, сообщением и временной меткой.

Внедрение зависимостей (Dependency Injection) обеспечивается контейнером Spring Beans через конструкторы (constructor injection с поддержкой Lombok `@RequiredArgsConstructor`), что повышает тестируемость и исключает циклические зависимости.
...
Все эндпоинты следуют единым соглашениям: 200 OK — успешное чтение, 201 Created — успешное создание ресурса с возвратом нового идентификатора, 204 No Content — успешное удаление, 400 Bad Request — ошибка валидации входных данных, 401 Unauthorized — отсутствие или невалидность JWT-токена, 403 Forbidden — недостаточно прав, 404 Not Found — ресурс не существует, 409 Conflict — нарушение бизнес-правил (например, превышение лимита мест), 500 Internal Server Error — непредвиденная серверная ошибка. Ответы на ошибки имеют единый формат (поля `code`, `message`, `timestamp`, опционально `fieldErrors`), формируемый единым обработчиком `GlobalExceptionHandler` через `@RestControllerAdvice`. Валидация запросов реализована через Jakarta Validation (`@Valid`, `@NotBlank`, `@Size`, `@Email`).

Документация API сформирована с помощью SpringDoc OpenAPI 2.6.0 (Swagger UI) и доступна по адресу `/swagger-ui.html` в ходе разработки. Каждая группа эндпоинтов аннотирована тегами с описанием на русском языке, что упрощает работу команды клиентской разработки и тестировщиков.
...
Рекомендательная система. Метод `getRecommendations()` ранжирует мероприятия по количеству добавлений в избранное (по убыванию), а при равенстве показателей — по дате создания. Данный алгоритм реализует упрощённую коллаборативную фильтрацию на основе агрегированного поведения пользователей и подходит для MVP-фазы; в дальнейшем (раздел 3.4) предусмотрено расширение алгоритма через таблицу `user_interests` для учёта индивидуальных предпочтений.

Управление транзакциями. Все операции, изменяющие данные, обёрнуты в транзакции через аннотацию `@Transactional` на уровне сервисного слоя. Для критичных бизнес-операций — оформление заказа, генерация QR-токенов, обработка вебхука платежа — используется уровень изоляции по умолчанию (READ_COMMITTED) с явным откатом при возникновении исключений, что гарантирует согласованность связанных данных в таблицах `orders`, `order_items`, `payments` и `tickets`.

Обработка ошибок. Класс `GlobalExceptionHandler`, помеченный аннотацией `@RestControllerAdvice`, перехватывает все необработанные исключения и преобразует их в унифицированные HTTP-ответы. Поддерживаются специализированные исключения предметной области: `ResourceNotFoundException` (404), `ValidationException` (400), `UnauthorizedActionException` (403), `BusinessRuleViolationException` (409), что позволяет клиентской части корректно интерпретировать ошибки и отображать пользователю релевантные сообщения.
...
Запуск приложения в среде разработки. Серверная часть запускается командой `mvn spring-boot:run` (Maven, порт 8080); клиентская часть — командой `npm run dev` (Vite, порт 5173); база данных — локальный экземпляр PostgreSQL 16 на порту 5432. Flyway автоматически применяет все миграции при каждом запуске серверной части. Конфигурация среды (SMTP, OAuth2, YooKassa, Яндекс Метрика) задаётся через файл `.env`, подключаемый к `application.yml`.
...
Приложение реализует три независимых пользовательских пространства с разными точками входа и навигацией. На рисунке 2.13 представлена схема навигации веб-приложения (навигационная карта / sitemap) в нотации иерархической диаграммы с группировкой по ролевым пространствам.
...
Клиентская часть организована по функционально-ориентированному принципу. Исходный код сосредоточен в директории `src/` и разделён на следующие модули:
1) `pages` — страницы приложения (более 30 экранов): публичные (главная, афиша, карточка мероприятия, карта), кабинеты жителя, организатора и администратора;
2) `components` — переиспользуемые UI-компоненты (более 50 элементов): доменные (`EventCard`, `StarRating`, `SessionCard`) и базовые элементы UI Kit;
3) `layouts` — шаблоны разметки `PublicLayout`, `OrganizerLayout`, `AdminLayout` с собственной навигацией и ролевой защитой;
4) `services` — слой взаимодействия с REST API (19 модулей-фасадов): `auth-service`, `event-service`, `order-service`, `notification-service` и др.;
5) `contexts` — глобальное состояние через React Context: `AuthContext` (текущий пользователь, JWT-токен), `CityContext` (выбранный город);
6) `hooks` — пользовательские хуки для повторно используемой логики;
7) `types` — TypeScript-интерфейсы и типы, синхронизированные с DTO серверной части;
8) `lib` — вспомогательные утилиты (форматирование дат, парсинг ошибок API, работа с JWT).

Управление серверным состоянием реализовано через TanStack React Query 5.83, что обеспечивает декларативное кэширование, автоматическое обновление при фокусе окна, оптимистичные обновления и фоновую синхронизацию. Локальное состояние форм управляется через React Hook Form 7.61 с валидацией Zod 3.25, обеспечивающей строгую типизацию схемы валидации совместно с TypeScript.
...
>Рисунок 2.22 — C4-диаграмма компонентов клиентской части
...
В разделе 2.1 спроектирована база данных, включающая 35 таблиц. Инфологическая модель формализует предметную область через 19 ключевых сущностей и связи между ними. Даталогическая модель реализована на СУБД PostgreSQL 16 с применением паттерна Soft Delete для пользователей, мероприятий, организаций и артистов, а также декларативных ограничений целостности через внешние ключи и CHECK-ограничения. Эволюция схемы управляется инструментом Flyway: восемь применённых миграций (V1–V8, таблица 2.3) последовательно вводят базовую схему, справочники, расширенные функции (интересы пользователей, полнотекстовый поиск, список ожидания, уведомления, промокоды, подписки на организации). Архитектура схемы поддерживает полный жизненный цикл мероприятия — от создания черновика до архивирования.

В разделе 2.2 разработана серверная часть на платформе Java 17 / Spring Boot 3. Архитектура системы описана с помощью нотации C4 Model на трёх уровнях: диаграмма контекста (рис. 2.4) отражает систему в окружении пользователей и внешних сервисов; диаграмма контейнеров (рис. 2.5) раскрывает внутренние компоненты и их взаимодействие; диаграмма компонентов (рис. 2.6) детализирует слои серверной части. Реализована многоуровневая (layered) архитектура с разделением ответственности на восемь пакетов: 43 JPA-сущности, репозитории Spring Data JPA, 33 сервиса бизнес-логики, 23 REST-контроллера, более 70 DTO, компоненты безопасности, конфигурации и обработчики исключений. REST API охватывает все функциональные требования и сгруппирован в 16 функциональных групп (таблица 2.2): аутентификация, мероприятия, сеансы, заказы, билеты, избранное, комментарии, публикации, кабинет организатора, платежи, справочники, панель администратора, список ожидания, промокоды, уведомления, подписки на организации. Реализована интеграция с пятью внешними сервисами: YooKassa (платежи), Яндекс Почта (SMTP), Google / Яндекс OAuth2 (аутентификация), Яндекс Карты (геолокация) и Яндекс Метрика (веб-аналитика трафика в панели администратора).

В разделе 2.3 разработан пользовательский интерфейс, включающий три независимых пространства: публичный сайт с афишей мероприятий, личный кабинет организатора с пошаговым мастером создания событий из шести этапов (Основная информация → Фотографии → Артисты → Сеансы → Билеты → Предпросмотр) и панель администратора с инструментами модерации. Бизнес-процесс публикации мероприятия формализован в виде BPMN-диаграммы (рис. 2.18), а жизненный цикл мероприятия — диаграммой состояний (рис. 2.19). Прецеденты системы по ролям описаны в Use Case-диаграмме (рис. 2.21), охватывающей 4 актёра и 20 прецедентов. Визуальная система обеспечивает адаптивную вёрстку (минимальная ширина 320 пикселей) и соответствует требованиям доступности WCAG 2.1 уровня AA.

В разделе 2.4 реализована клиентская часть в виде SPA на базе React 18 и TypeScript 5.8. Архитектура клиентской части описана посредством C4-диаграммы компонентов (рис. 2.22) и включает семь логических слоёв: инициализация, маршрутизация, контексты, страницы, UI-компоненты, сервисный слой и HTTP-клиент. Разработан унифицированный HTTP-клиент с автоматическим управлением JWT-токеном, сервисный слой по паттерну «Фасад» (19 модулей), механизм защищённых маршрутов для ролевого разграничения доступа. Управление серверным состоянием реализовано через TanStack React Query, локальным состоянием форм — через React Hook Form в связке с Zod-валидацией.

Таким образом, реализованное веб-приложение представляет собой полноценную многоролевую систему, автоматизирующую ключевые процессы организации, продвижения и посещения культурных мероприятий в малом городе. Все технические решения направлены на достижение баланса между функциональной полнотой, удобством использования и надёжностью системы. Вопросы тестирования, обеспечения информационной безопасности, сценарии использования и рекомендации по развитию рассматриваются в третьей главе.
...
Тестирование реализованного веб-приложения проводилось с целью верификации соответствия функциональным и нефункциональным требованиям, сформулированным в разделе 1.5, а также валидации корректности реализованных бизнес-сценариев. Стратегия тестирования построена в соответствии с моделью «пирамиды тестирования» [54] и охватывает четыре уровня: функциональное тестирование пользовательских сценариев (acceptance testing), нагрузочное тестирование серверной части (performance testing), проверка информационной безопасности по методологии OWASP Top 10 [48], а также аудит доступности и адаптивности клиентской части (accessibility testing).

Перечень используемого инструментария представлен в таблице 3.0.

Таблица 3.0 — Инструментарий тестирования

| Уровень | Инструмент | Назначение |
|---|---|---|
| Функциональное | Postman, ручное тестирование в браузере | Проверка REST-эндпоинтов и пользовательских сценариев |
| Нагрузочное | Apache JMeter [49] | Симуляция параллельных пользователей, измерение времени отклика |
| Безопасность | OWASP ZAP, ручной аудит кода | Сканирование на уязвимости категорий A01–A10 OWASP Top 10 |
| Доступность | Google Lighthouse, axe DevTools | Проверка соответствия WCAG 2.1 уровня AA |
| Адаптивность | Chrome DevTools Device Toolbar | Эмуляция мобильных устройств от 320 пикселей |

3.1.2 Функциональное тестирование

Функциональное тестирование охватило ключевые сценарии всех трёх ролей и проводилось вручную через браузерный интерфейс с дополнительной проверкой REST-эндпоинтов через Postman. Результаты сведены в таблицы 3.1 и 3.2 и фиксируют ожидаемое и фактическое поведение системы.
...
| A06 | Уязвимые компоненты | Регулярное обновление зависимостей через Maven; используются актуальные версии Spring Boot 3.3.4, JJWT 0.12.6 |
...
В системе обрабатываются следующие категории персональных данных пользователей: ФИО, адрес электронной почты, номер телефона, город проживания, история заказов и регистраций. Меры защиты соответствуют требованиям Федерального закона № 152-ФЗ:
1) пароли хранятся исключительно в виде хешей BCrypt — соответствие принципу обеспечения конфиденциальности;
2) реализован паттерн Soft Delete — пользователь может запросить удаление учётной записи, при этом сохраняется аудиторская история;
3) email-верификация подтверждает принадлежность учётной записи владельцу адреса;
4) доступ к персональным данным разграничен по ролям, журнал административных действий фиксирует факты обработки данных;
5) передача персональных данных третьим сторонам ограничена платёжным шлюзом YooKassa и SMTP-сервером, что соответствует целям обработки.
...
2)	запустить серверную часть: `mvn spring-boot:run` из директории `backend/` — Flyway автоматически применит все миграции (V1–V8), порт 8080.
3)	запустить клиентскую часть: `npm run dev` из директории `frontend/` — Vite поднимет dev-сервер на порту 5173.
...
1)	контейнеризация с помощью Docker и Docker Compose — автоматизация развёртывания в единую команду (`docker compose up`) с полной изоляцией зависимостей и воспроизводимым окружением;
...
Стратегия тестирования (раздел 3.1) построена в соответствии с моделью «пирамиды тестирования» и охватила четыре уровня: функциональное, нагрузочное, тестирование безопасности и аудит доступности. Использованный инструментарий (Postman, Apache JMeter, OWASP ZAP, Google Lighthouse, axe DevTools) представлен в таблице 3.0.

Функциональное тестирование в объёме 18 тест-кейсов, охватывающих сценарии всех трёх ролей (житель, организатор, администратор), подтвердило корректность реализации всех заявленных функциональных требований: все тест-кейсы успешно пройдены. В том числе верифицированы расширенные возможности системы: список ожидания на сеансы с превышенным лимитом и email-уведомлением первому в очереди (WaitlistService), система промокодов трёх видов — фиксированная скидка, процентная скидка и бесплатный билет (PromoCodeService), а также механизм подписки пользователей на организации с уведомлениями о новых мероприятиях.

Нагрузочное тестирование (500 виртуальных пользователей, Apache JMeter, смешанный профиль запросов) показало среднее время отклика 1 320 мс и долю ошибок 0,4% при использовании CPU 62%, что укладывается в нефункциональный норматив не более 2 с (таблица 1.8) и подтверждает запас по нагрузке. Аудит инструментами Lighthouse и axe DevTools подтвердил соответствие WCAG 2.1 уровня AA по пяти ключевым страницам и адаптивность интерфейса в диапазоне разрешений от 320 до 1440 пикселей.

Анализ информационной безопасности (раздел 3.2) сгруппирован по четырём подразделам: аутентификация и управление сессиями, авторизация и ролевая модель, защита от атак по OWASP Top 10 (со сводной таблицей 3.4 покрытия категорий A01–A10), защита персональных данных по 152-ФЗ. Реализованный комплекс мер — BCrypt, stateless JWT, параметризованные запросы JPA, ролевое разграничение через Spring Security, Soft Delete и журнал административных действий — обеспечивает многослойную защиту системы и соответствие требованиям российского законодательства.

Разработано руководство пользователя (раздел 3.3) с описанием порядка развёртывания системы в среде разработки и трёх детализированных сценариев работы для всех ролей. Сформированы семь приоритетных направлений дальнейшего развития (раздел 3.4): контейнеризация, CI/CD, перевод в production, мобильное приложение, Telegram-бот, полнотекстовый поиск на уровне СУБД, горизонтальное масштабирование. Рекомендации опираются на уже заложенную в архитектуре и схеме базы данных инфраструктуру и обеспечивают плавный переход от MVP к промышленной эксплуатации.
...
53. ISO 9241-210:2019. Ergonomics of human-system interaction — Part 210: Human-centred design for interactive systems. — ISO, 2019.
