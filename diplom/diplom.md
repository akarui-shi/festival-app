...
3)	service — бизнес-логика: EventService, OrderService, PaymentService, NotificationService и иные;
...
8)	exception — пользовательские исключения и единый обработчик ошибок (@ControllerAdvice).
...
Развёртывание посредством Docker Compose. Приложение контейнеризировано с помощью Docker; конфигурация Docker Compose объединяет три сервиса: PostgreSQL (db), Spring Boot (серверная часть) и React-клиент (клиентская часть). Единственная команда docker compose up поднимает все сервисы с автоматическим применением миграций Flyway при старте серверной части [45].
...
Реализованные меры защиты охватывают основные категории угроз по методологии OWASP Top 10 [48]: хранение паролей в виде хешей BCrypt, stateless JWT-аутентификация, параметризованные запросы через JPA (защита от SQL-инъекций), ролевое разграничение доступа, Soft Delete для обеспечения историчности данных. Система соответствует требованиям Федерального закона № 152-ФЗ «О персональных данных» [50].

3.2.1 Нагрузочное тестирование

Нагрузочное тестирование проводилось с использованием Apache JMeter [49] при 500 виртуальных пользователях. Результаты: среднее время отклика — 1320 мс, доля ошибок — 0,4%, что укладывается в нефункциональный норматив не более 2 с (таблица 1.8). Lighthouse-аудит подтвердил соответствие WCAG 2.1 уровня AA по всем ключевым страницам.

3.3 Руководство пользователя

3.3.1 Развёртывание системы

Система контейнеризирована с помощью Docker; конфигурация Docker Compose [45] объединяет три сервиса: PostgreSQL (db), Spring Boot серверная часть (backend) и React-клиент (клиентская часть). Команда docker compose up поднимает все сервисы с автоматическим применением миграций Flyway.

3.3.2 Сценарий 1: поиск и запись на мероприятие (роль — Житель)

Порядок действий:
1.	Открыть главную страницу и выбрать город.
2.	Перейти в раздел «Афиша», применить фильтры по дате, категории или площадке.
3.	Выбрать мероприятие, ознакомиться с карточкой, нажать «Записаться».
4.	После записи на email поступает QR-билет.
...
2.	Последовательно заполнить 6 шагов мастера: основная информация, расписание, площадка, билеты, медиафайлы, публикация.
3.	Нажать «Отправить на модерацию».
4.	После одобрения администратором мероприятие публикуется в афише.
      
3.3.4 Сценарий 3: модерация и управление системой (роль — Администратор)

Порядок действий:
1. Войти в систему и перейти в «Панель администратора» → «Обзор системы».
...
1)	список ожидания на сеансы — таблица session_waitlist уже включена в схему базы данных;
2)	Push-уведомления через Web Push API — таблица notifications предусмотрена для хранения очереди уведомлений;
3)	персонализированные рекомендации — таблица user_interests заложена для хранения предпочтений пользователей;
4)	промокоды — таблица promo_codes предусмотрена для системы скидочных промокодов;
5)	мобильное приложение для iOS и Android на базе существующего REST API;
6)	масштабирование на несколько городов — архитектура изначально спроектирована с поддержкой справочника городов.
...
Разработано руководство пользователя с описанием порядка развёртывания системы и трёх ключевых пользовательских сценариев. Сформированы рекомендации по дальнейшему развитию в шести направлениях, опирающиеся на уже заложенную в архитектуре и схеме базы данных инфраструктуру.
