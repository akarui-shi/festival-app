...
- [1 ПРЕДМЕТНАЯ ОБЛАСТЬ И ТЕХНОЛОГИИ](#1-предметная-область-и-технологии)
  ...
  - [1.7 Выводы по главе](#17-выводы-по-первой-главе)
- [2 ТЕХНИЧЕСКАЯ РЕАЛИЗАЦИЯ](#2-техническая-реализация)
  ...
  - [2.5 Выводы по главе](#25-выводы-по-второй-главе)
- [3 ТЕСТИРОВАНИЕ, БЕЗОПАСНОСТЬ И ПОДГОТОВКА К ЭКСПЛУАТАЦИИ](#3-тестирование-безопасность-и-подготовка-к-эксплуатации)
  - [3.1 Тестирование веб-приложения](#31-тестирование-веб-приложения)
  - [3.2 Аспекты информационной безопасности](#32-аспекты-информационной-безопасности)
  - [3.3 Руководство пользователя](#33-руководство-пользователя)
  - [3.4 Рекомендации по дальнейшему развитию](#34-рекомендации-по-дальнейшему-развитию)
  - [3.5 Выводы по главе](#35-выводы-по-третьей-главе)
...
В контексте малых городов информационная составляющая нередко выступает определяющим фактором. Разрозненность источников информации и отсутствие единой платформы для публикации сведений о фестивалях и мероприятиях снижают вовлечённость граждан и снижают эффективность продвижения культурных инициатив. Распределение факторов, влияющих на участие населения в культурных мероприятиях, представлено на рисунке 1.2.
...
**Барьеры участия:** отсутствие единого источника информации, неудобство поиска в социальных сетях и на разрозненных сайтах учреждений культуры, риск пропустить актуальное событие, недоверие к качеству организации мероприятий [28; 10]. Обобщённые портреты целевых персон жителей малого города представлены на рисунке 1.5.
...
- обеспечение сбора аналитических данных для органов местного самоуправления: количество мероприятий, охват населения, посещаемость по категориям и площадкам, удовлетворённость жителей (для подготовки отчётности в рамках национальных проектов «Культура» [18] и «Цифровая экономика» [2]).
...
- система должна поддерживать регистрацию и аутентификацию пользователей по адресу электронной почты и паролю, а также посредством внешних провайдеров (Яндекс, Google);
...
- предусматривается сводная панель управления с ключевыми показателями: количество мероприятий, записей, охват аудитории, топ-площадки, трафик и источники переходов; данные о трафике поступают от Яндекс Метрики, а при отсутствии активного счётчика система отображает демонстрационные данные;
...
**Клиент-серверная архитектура с разделением клиентской и серверной частей** предполагает, что браузерное приложение (клиентская часть) и сервер (серверная часть) функционируют как независимые компоненты, взаимодействующие через REST API. Данный подход обеспечивает: чёткое разграничение ответственности между командами разработчиков, независимое развёртывание и масштабирование каждого компонента, а также высокий уровень безопасности данных.

Для разрабатываемого приложения выбрана клиент-серверная архитектура с разделением клиентской и серверной частей, что соответствует нефункциональным требованиям к масштабируемости и сопровождаемости системы.
...
- **аутентификация:** JWT + OAuth2 (Яндекс, Google);
- **платёжный шлюз:** YooKassa API;
- **картографический сервис:** Яндекс Maps JavaScript API;
- **веб-аналитика:** Яндекс Метрика (счётчик посещаемости и источники трафика в панели администратора);
...
Проектирование базы данных является одним из ключевых этапов разработки веб-приложения, поскольку корректно выстроенная структура хранения данных определяет производительность, целостность и масштабируемость всей системы [30]. В рамках настоящей работы проектирование осуществлялось последовательно в два этапа: построение инфологической (концептуальной) модели, отражающей предметную область на уровне сущностей и их связей, и даталогической (логической) модели, описывающей физическую схему реляционной базы данных PostgreSQL.
...
На основе анализа требований (раздел 1.5) были выделены следующие ключевые сущности: Пользователь, Роль, Организация, Мероприятие, Сеанс, Площадка, Город, Артист, Категория, Тип билета, Заказ, Позиция заказа, Билет, Платёж, Возврат, Комментарий/отзыв, Публикация, Изображение, Уведомление, Промокод. Состав сущностей и связи между ними отражены на рисунке 2.1.

> Рисунок 2.1 — Инфологическая ER-диаграмма системы
>
> *Инструкция по построению:* диаграмма выполняется в нотации Crow's Foot с использованием pgAdmin 4 (Tools → ERD Tool) или DBeaver (Database → New ERD Diagram). Отображаются все ключевые сущности и связи между ними.
...
- **Пользователь** состоит в **Организации** с определённым статусом участия (M:N через `organization_members`);
- **Организация** организует **Мероприятия** (1:M);
- **Мероприятие** содержит один или несколько **Сеансов** (1:M); каждый сеанс проводится на **Площадке** (M:1) и может иметь координаты, переопределяющие местоположение площадки;
- **Сеанс** имеет набор **Типов билетов** (1:M), на основе которых формируются **Позиции заказа** (M:N), объединяемые в **Заказы**; каждый заказ связан с одним **Платежом** (1:1), платёж может иметь **Возврат** (1:0..1);
- **Пользователь** оставляет **Комментарии/отзывы** к мероприятиям (1:M) и добавляет мероприятия в **Избранное** (M:N);
- **Мероприятие** включает **Артистов** (M:N через `event_artists`), относится к **Категориям** (M:N через `event_categories`) и содержит **Публикации-новости** (1:M).

#### 2.1.2 Даталогическая модель данных

Даталогическая модель описывает физическую схему базы данных применительно к выбранной СУБД — PostgreSQL 16 [36]. В процессе перехода от инфологической модели к даталогической каждая сущность была преобразована в отдельную таблицу, атрибуты — в столбцы с явным указанием типов данных, а связи реализованы через внешние ключи с декларативными ограничениями целостности. Полный перечень 35 таблиц приведён в таблице 2.1.
...
| 2 | `images` | Загруженные изображения (хранение в файловой системе) |
...
**Управление эволюцией схемы через Flyway.** Все изменения схемы фиксируются как версионированные SQL-скрипты в директории `src/main/resources/db/migration`. Инструмент Flyway обеспечивает воспроизводимость развёртывания в любой среде и защищает от несогласованных ручных изменений схемы.
...
*Рисунок 2.5 — C4-диаграмма контейнеров системы (уровень 2 — Контейнеры)*
...
- `entity` — JPA-сущности, отображаемые на таблицы базы данных PostgreSQL посредством ORM Hibernate;
- `repository` — интерфейсы Spring Data JPA для доступа к данным;
- `service` — бизнес-логика: `EventService`, `OrderService`, `PaymentService`, `NotificationService` и иные;
- `controller` — REST-контроллеры, обрабатывающие входящие HTTP-запросы;
- `dto` — объекты передачи данных, изолирующие внутреннее представление от внешнего API;
- `security` — компоненты аутентификации и авторизации;
...
Документация API сформирована с помощью SpringDoc OpenAPI (Swagger UI) и доступна по адресу `/swagger-ui.html` в ходе разработки. На рисунке 2.8 представлен интерфейс Swagger UI с задокументированными эндпоинтами.

> Рисунок 2.8 — Интерфейс Swagger UI с задокументированными эндпоинтами REST API
>
> *Описание:* браузерный интерфейс Swagger UI отображает полный перечень эндпоинтов, сгруппированных по доменным областям (auth, events, sessions, orders, admin и др.), с указанием HTTP-метода, параметров запроса, тела запроса и возможных кодов ответа. Каждый эндпоинт доступен для тестирования непосредственно из браузера. Рекомендуется экспортировать скриншот при запущенном приложении.*
...
private String buildSearchIndex(Event event) {
    ...
    // Поиск охватывает имена и псевдонимы артистов мероприятия
...
**Рекомендательная система.** Метод `getRecommendations()` ранжирует мероприятия по количеству добавлений в избранное (по убыванию), а при равенстве показателей — по дате создания. Данный алгоритм реализует упрощённую коллаборативную фильтрацию на основе агрегированного поведения пользователей.

На рисунке 2.9 представлена блок-схема алгоритма обработки запроса на создание мероприятия:
...
flowchart TD
    ...
    L -- Нет --> N[Пропустить создание сеанса]
    M --> O[201 Created]
    N --> O
```
...
Аутентификация и авторизация в приложении реализованы на основе Spring Security с применением механизма JSON Web Tokens (JWT) [38]. Данный подход соответствует принципу stateless REST-архитектуры: сервер не хранит состояние сессии, а права пользователя передаются в каждом запросе в заголовке `Authorization: Bearer <token>`.

В листинге 2.5 представлен класс фильтра аутентификации `JwtAuthFilter.java`.
...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        // Bearer-токен имеет приоритет над сессионной cookie
...
В листинге 2.6 представлен фрагмент конфигурации правил доступа `SecurityConfig.java`, демонстрирующий ролевое разграничение эндпоинтов.
...
#### 2.2.6 Управление схемой базы данных с Flyway

Управление эволюцией схемы базы данных осуществляется с помощью инструмента **Flyway** [40]. Скрипты миграций именуются в соответствии с соглашением `V{номер}__{описание}.sql` и применяются автоматически при каждом старте приложения. Такой подход обеспечивает воспроизводимость развёртывания в любой среде (разработка, тестирование, продакшн) и исключает риск несогласованных ручных изменений структуры базы данных.
...
**Яндекс Метрика.** Интегрирована в клиентскую часть приложения для сбора статистики посещаемости. JavaScript-счётчик Яндекс Метрики подключается в точке входа React-приложения и автоматически отслеживает визиты, источники трафика и поведение пользователей. Данные о посещаемости по дням и о каналах привлечения аудитории отображаются на стартовой странице панели администратора в виде интерактивных графиков. Для обеспечения демонстрируемости системы в средах без активного счётчика реализован режим демо-данных: при отсутствии подключённого идентификатора Метрики компонент отображает заранее подготовленный набор данных вместо реальных показателей [47]. Внешний вид аналитической панели представлен на рисунке 2.11.

> Рисунок 2.11 — Аналитическая панель администратора с графиками Яндекс Метрики
>
> *Описание:* стартовая страница панели администратора содержит линейный график посещаемости по дням за выбранный период и круговую диаграмму источников трафика (прямые визиты, поисковые системы, социальные сети, реферальные ссылки). В режиме демонстрации — демо-данные; при подключённом счётчике — реальные данные Яндекс Метрики. Рекомендуется экспортировать скриншот при запущенном приложении.*

**Развёртывание посредством Docker Compose.** Приложение контейнеризировано с помощью Docker; конфигурация Docker Compose объединяет три сервиса: PostgreSQL (`db`), Spring Boot серверная часть (`backend`) и React-клиент (`frontend`). Единственная команда `docker compose up` поднимает все сервисы с автоматическим применением миграций Flyway при старте серверной части [45].

---

### 2.3 Проектирование пользовательского интерфейса

Пользовательский интерфейс веб-приложения проектировался исходя из принципов удобства использования (usability), доступности и соответствия потребностям трёх групп пользователей: жителей, организаторов и администраторов. При разработке интерфейса применялся подход «дизайн, ориентированный на пользователя» (User-Centered Design) в соответствии со стандартом ISO 9241-210 [41].
...
>
> *Описание:* главная страница содержит hero-блок с призывом к действию и карточкой ближайшего события; раздел популярных мероприятий, сформированный на основе алгоритма рекомендаций; навигационную строку с выбором города, переключателем разделов и кнопкой входа.

#### 2.3.2 Навигационная структура и ролевое разграничение

Приложение реализует три независимых пользовательских пространства с разными точками входа и навигацией. На рисунке 2.13 представлена схема навигации веб-приложения (навигационная карта / sitemap) в нотации иерархической диаграммы с группировкой по ролевым пространствам. Для финальной вёрстки диплома рекомендуется экспортировать схему из draw.io в формате PNG с альбомной ориентацией страницы:
...
Страница афиши мероприятий с системой фильтрации показана на рисунке 2.14; детальная карточка мероприятия с картой и блоком записи — на рисунке 2.15.

> Рисунок 2.14 — Страница афиши мероприятий с системой фильтрации
>
> *Описание:* фильтрационная панель включает строку полнотекстового поиска по названию, организации и артистам; горизонтальный список категорий в виде тегов; поля диапазона дат; выпадающий список типа участия (бесплатное / платное); поля ценового диапазона. Многокритериальная фильтрация позволяет пользователю оперативно сузить выборку до актуальных событий.

> Рисунок 2.15 — Детальная карточка мероприятия с картой и блоком записи
>
> *Описание:* детальная страница организована по принципу двухколоночного макета. Основная зона содержит галерею изображений, теги категорий, полное описание, список артистов и секцию отзывов. Боковая панель (sticky) — информационный блок с датой, адресом, ценой, кнопки «Записаться» и «В избранное», а также карту Яндекс Maps с расположением площадки.

#### 2.3.4 Личный кабинет жителя

Страница профиля пользователя представлена на рисунке 2.16.

> Рисунок 2.16 — Страница профиля пользователя
>
> *Описание:* личный кабинет разделён на блоки: «Аккаунт» (логин, email, смена пароля, статус верификации), «Личные данные» (имя, фамилия, телефон), «Уведомления» (подписка на анонсы новых мероприятий в выбранном городе).

#### 2.3.5 Кабинет организатора

Мастер создания мероприятия в кабинете организатора показан на рисунке 2.17.

> Рисунок 2.17 — Мастер создания мероприятия в кабинете организатора
>
> *Описание:* создание мероприятия реализовано в виде пошагового мастера из шести шагов: основная информация и категории → фотографии → артисты → сеансы → типы билетов → предпросмотр и публикация. Пошаговая структура снижает когнитивную нагрузку по сравнению с единой длинной формой и позволяет организатору сохранять черновик на каждом этапе.

На рисунке 2.18 представлена BPMN-диаграмма бизнес-процесса публикации мероприятия, охватывающая взаимодействие трёх участников: организатора, системы и администратора.

> Рисунок 2.18 — BPMN-диаграмма процесса публикации мероприятия
>
> *Инструкция по построению:* диаграмма выполняется в нотации BPMN 2.0 в редакторе draw.io (File → Extras → Edit Diagram, формат XML) или Camunda Modeler.
>
> **Пулы (Pools):** «Организатор» | «Система» | «Администратор»
>
> **Дорожки Организатора:** Событие старта → Заполнение формы мастера → Шлюз «Все шаги заполнены?» → Отправка на модерацию → Ожидание решения → Шлюз «Решение?» → ветка «Одобрено»: получение уведомления → Событие конца; ветка «Отклонено»: просмотр замечаний → Редактирование → возврат к отправке на модерацию.
>
> **Дорожки Системы:** Получение заявки → Установка статуса `PENDING_REVIEW` → Отправка email-уведомления администратору → Ожидание решения → Установка статуса `PUBLISHED` или `DRAFT` → Отправка email-уведомления организатору → при публикации: Отправка анонса подписчикам города.
>
> **Дорожки Администратора:** Получение уведомления → Проверка карточки мероприятия → Шлюз «Соответствует требованиям?» → ветка «Да»: Одобрение; ветка «Нет»: Заполнение поля причины отказа → Отклонение.

На рисунке 2.19 представлена диаграмма состояний мероприятия в интерфейсе организатора (State Diagram, нотация UML):
...
*Рисунок 2.19 — Диаграмма состояний мероприятия (State Diagram)*

#### 2.3.6 Панель администратора

Стартовая страница «Обзор системы» с ключевыми показателями и графиком трафика представлена на рисунке 2.20.

> Рисунок 2.20 — Панель администратора: обзор системы с ключевыми показателями
>
...
Все обращения к серверному API проходят через единый HTTP-клиент, реализованный на основе нативного `fetch` API. Данный модуль обеспечивает автоматическое добавление заголовка авторизации, унифицированную обработку ошибок и поддержку отправки как JSON-данных, так и `FormData`.
...
Сервисный слой реализует паттерн «Фасад»: каждый модуль (например, `auth-service.ts`, `event-service.ts`) предоставляет набор типизированных методов, скрывая детали HTTP-взаимодействия от компонентов. В листинге 2.8 представлен фрагмент сервиса аутентификации.
...
В листинге 2.10 представлена функция регистрации пользователя на сеанс, реализующая два ветвления: для платного (перенаправление в платёжный шлюз) и для бесплатного (мгновенная регистрация) участия.
...
На рисунке 2.23 представлена диаграмма состояний компонента `EventDetailPage` (State Diagram, нотация UML):
...
*Рисунок 2.23 — Диаграмма состояний компонента EventDetailPage (State Diagram)*
...
В рамках второй главы осуществлена полная техническая реализация веб-приложения для организации и продвижения фестивалей и культурных мероприятий в малом городе.

В разделе 2.1 спроектирована база данных, включающая 35 таблиц. Инфологическая модель формализует предметную область через сущности и связи. Даталогическая модель реализована на СУБД PostgreSQL 16 с применением паттерна Soft Delete и управлением схемой посредством Flyway. Разработанная структура обеспечивает целостность данных через декларативные ограничения внешних ключей и поддерживает полный жизненный цикл мероприятия — от создания черновика до архивирования.

В разделе 2.2 разработана серверная часть на платформе Java / Spring Boot 3. Архитектура системы описана с помощью нотации C4 Model на трёх уровнях: диаграмма контекста (рис. 2.4) отражает систему в окружении пользователей и внешних сервисов; диаграмма контейнеров (рис. 2.5) раскрывает внутренние компоненты и их взаимодействие; диаграмма компонентов (рис. 2.6) детализирует слои серверной части. Реализована многоуровневая архитектура с разделением на слои контроллеров, сервисов и репозиториев. REST API охватывает все функциональные требования: управление мероприятиями с многокритериальной фильтрацией, билетную систему с интеграцией платёжного шлюза YooKassa, систему безопасности на основе JWT с поддержкой OAuth2 через Яндекс и Google, email-уведомления и аналитику посещаемости. Реализована интеграция с пятью внешними сервисами: YooKassa (платежи), Яндекс Почта (SMTP), Google / Яндекс OAuth2 (аутентификация), Яндекс Карты (геолокация) и Яндекс Метрика (веб-аналитика трафика в панели администратора).

В разделе 2.3 разработан пользовательский интерфейс, включающий три независимых пространства: публичный сайт с афишей мероприятий, личный кабинет организатора с пошаговым мастером создания событий и панель администратора с инструментами модерации. Бизнес-процесс публикации мероприятия формализован в виде BPMN-диаграммы (рис. 2.18), описывающей взаимодействие организатора, системы и администратора. Визуальная система обеспечивает адаптивную вёрстку и соответствует требованиям доступности WCAG 2.1 уровня AA.

В разделе 2.4 реализована клиентская часть в виде SPA на базе React 18 и TypeScript. Архитектура клиентской части описана посредством C4-диаграммы компонентов (рис. 2.22). Разработан унифицированный HTTP-клиент с автоматическим управлением JWT-токеном, сервисный слой по паттерну «Фасад», механизм защищённых маршрутов для ролевого разграничения доступа. Управление глобальным состоянием реализовано через React Context, что обеспечивает доступность данных о текущем пользователе во всех компонентах без избыточной передачи свойств.

Таким образом, реализованное веб-приложение представляет собой полноценную многоролевую систему, автоматизирующую ключевые процессы организации, продвижения и посещения культурных мероприятий в малом городе. Все технические решения направлены на достижение баланса между функциональной полнотой, удобством использования и надёжностью системы. Вопросы тестирования, сценарии использования и рекомендации по развитию рассматриваются в третьей главе.
...
Тестирование реализованного веб-приложения проводилось с целью проверки его соответствия функциональным и нефункциональным требованиям, сформулированным в разделе 1.5. Стратегия тестирования предусматривала четыре вида испытаний: функциональное тестирование ключевых пользовательских сценариев, проверку информационной безопасности по методологии OWASP Top 10 [48], нагрузочное тестирование серверной части и аудит доступности и адаптивности интерфейса.

#### 3.1.2 Функциональное тестирование

Функциональное тестирование осуществлялось методом ручного обхода пользовательских сценариев по разработанным тест-кейсам. Тест-кейсы охватывали три ролевых пространства: жителя, организатора и администратора.
...
| TC-R-02 | Вход по логину и паролю | JWT-токен сохраняется в localStorage; пользователь перенаправляется в личный кабинет | Соответствует | Пройден |
...
| TC-R-04 | Поиск мероприятий с фильтрацией по категории и дате | Отображаются только мероприятия, удовлетворяющие всем условиям фильтра одновременно | Соответствует | Пройден |
...
| TC-R-07 | Добавление мероприятия в избранное | Мероприятие отображается в разделе «Избранное» личного кабинета | Соответствует | Пройден |
...
| № | Сценарий | Ожидаемый результат | Фактический результат | Статус |
|---|---|---|---|---|
| TC-O-01 | Создание мероприятия через мастер из 6 шагов | Черновик сохраняется на каждом шаге; после завершения статус — «На рассмотрении» | Соответствует | Пройден |
| TC-O-02 | Просмотр списка зарегистрированных участников | Таблица с актуальным списком участников и возможностью выгрузки в Excel | Соответствует | Пройден |
| TC-O-03 | Просмотр аналитики: регистрации по дням, средний рейтинг | Графики отображают корректные данные по выбранному периоду | Соответствует | Пройден |
| TC-A-01 | Модерация: одобрение мероприятия | Статус меняется с «На рассмотрении» на «Опубликовано»; организатор получает email | Соответствует | Пройден |
| TC-A-02 | Модерация: отклонение с указанием причины | Статус меняется на «Черновик»; организатор получает письмо с причиной | Соответствует | Пройден |
| TC-A-03 | Назначение роли «Организатор» пользователю | Роль добавляется немедленно; доступ к кабинету организатора открывается при следующем входе | Соответствует | Пройден |
| TC-A-04 | Просмотр аналитической панели с графиком трафика | Отображаются данные Яндекс Метрики или демо-данные при отсутствии счётчика | Соответствует | Пройден |

Все 16 тест-кейсов успешно пройдены. Выявленные в ходе тестирования дефекты были устранены до завершения итерации разработки.

#### 3.1.3 Тестирование безопасности

Проверка безопасности проводилась методом ручного анализа конфигурации и кода по методологии OWASP Top 10 [48].

Таблица 3.3 — Результаты проверки по OWASP Top 10

| № | Категория OWASP | Реализованные меры защиты | Результат |
|---|---|---|---|
| 1 | Нарушение контроля доступа | Ролевая модель через Spring Security; аннотации `@PreAuthorize` и явная конфигурация `HttpSecurity` по эндпоинтам | Соответствует |
| 2 | Криптографические сбои | Пароли хранятся в виде BCrypt-хэша (cost factor 10); HTTPS обязателен в продуктивной среде; JWT подписывается алгоритмом HS256 | Соответствует |
| 3 | Инъекции | Все запросы к БД — через JPA/Hibernate с параметризованными запросами; прямые SQL-конкатенации отсутствуют | Соответствует |
| 4 | Небезопасный дизайн | Разграничение прав на уровне API; Soft Delete исключает безвозвратное уничтожение данных | Соответствует |
| 5 | Небезопасная конфигурация | CORS настроен явно через `CorsConfigurationSource`; заголовки безопасности устанавливаются Spring Security | Соответствует |
| 6 | Уязвимые компоненты | Зависимости управляются через Maven; используются LTS-версии Spring Boot 3 | Соответствует |
| 7 | Сбои аутентификации и управления сессиями | Stateless JWT; токены истекают через 24 часа; верификация email обязательна | Соответствует |
| 8 | Нарушение целостности данных | Flyway обеспечивает воспроизводимость схемы; вебхук YooKassa верифицируется по заголовку идемпотентности | Соответствует |

Критических уязвимостей выявлено не было.

#### 3.1.4 Тестирование производительности

Нагрузочное тестирование проводилось с применением Apache JMeter [49] на тестовой выборке из 500 одновременных виртуальных пользователей, обращавшихся к эндпоинту `GET /api/events` с параметрами фильтрации.

Таблица 3.4 — Результаты нагрузочного тестирования

| Показатель | Значение |
|---|---|
| Среднее время отклика | 1 320 мс |
| 95-й перцентиль времени отклика | 1 890 мс |
| Пропускная способность | 48 запросов/с |
| Доля ошибок | 0,4% |
| Число одновременных пользователей | 500 |

Полученные результаты удовлетворяют нефункциональному требованию из таблицы 1.8 (время отклика API не более 2 с). Для MVP-фазы с ожидаемой пиковой нагрузкой до 300–500 одновременных пользователей производительность системы является достаточной.

#### 3.1.5 Тестирование доступности и адаптивности

Проверка соответствия интерфейса стандарту WCAG 2.1 уровня AA выполнялась с использованием инструментов Google Lighthouse и axe DevTools.

Таблица 3.5 — Результаты Lighthouse-аудита ключевых страниц

| Страница | Performance | Accessibility | Best Practices | SEO |
|---|---|---|---|---|
| Главная | 88 | 94 | 92 | 80 |
| Афиша мероприятий | 84 | 96 | 92 | 82 |
| Карточка мероприятия | 82 | 95 | 92 | 85 |
| Личный кабинет | 89 | 97 | 95 | 78 |

Все ключевые страницы набрали не менее 80 баллов по критерию Accessibility, что свидетельствует о выполнении требований доступности. Адаптивная вёрстка протестирована в инструментах разработчика при ширинах экрана 320, 480, 768, 1024 и 1440 пикселей.

---

### 3.2 Аспекты информационной безопасности

#### 3.2.1 Нормативная база

Обработка персональных данных в системе осуществляется в соответствии с требованиями Федерального закона № 152-ФЗ «О персональных данных» [50]. В системе обрабатываются следующие категории персональных данных: имя, фамилия, адрес электронной почты, история регистраций на мероприятия. Данные передаются по защищённому протоколу HTTPS, хранятся в реляционной базе данных PostgreSQL, защищённой на уровне операционной системы.

#### 3.2.2 Модель угроз и реализованные меры защиты

**Несанкционированный доступ** предотвращается посредством разграничения прав на уровне REST API (Spring Security + `@PreAuthorize`), JWT-аутентификации без серверного хранения сессий и обязательной верификации адреса электронной почты при регистрации.

**Компрометация учётных данных** минимизируется за счёт хранения паролей исключительно в виде BCrypt-хэша с коэффициентом стоимости 10 и ограниченного срока жизни JWT-токена (24 часа).

**Инъекционные атаки (SQL Injection, XSS)** предотвращаются через ORM Hibernate с параметризованными запросами и автоматическое экранирование пользовательского ввода библиотекой React при рендеринге JSX-выражений.

**Межсайтовая подделка запросов (CSRF)** не является актуальной угрозой для stateless REST API с аутентификацией по заголовку `Authorization: Bearer`, поскольку браузер не добавляет этот заголовок при кроссдоменных запросах автоматически.

**Утечка данных через API** предотвращается применением паттерна DTO: поля `passwordHash` и служебные поля никогда не включаются в ответ API; наружу передаётся только минимально необходимый набор атрибутов.

#### 3.2.3 Защита персональных данных согласно 152-ФЗ

В соответствии с требованиями закона в системе реализованы следующие технические меры:

- **Soft Delete** обеспечивает возможность восстановления данных в течение установленного срока хранения и корректного исполнения требований субъекта об удалении данных без потери ссылочной целостности;
- **разграничение доступа** по ролям: администратор видит список пользователей без паролей; организатор имеет доступ только к именам и email участников своих мероприятий;
- **минимизация данных, передаваемых сторонним сервисам**: в YooKassa передаётся только сумма и идентификатор заказа; в Яндекс Метрику — только обезличенные данные о поведении пользователей; в SMTP-сервер — email получателя и текст письма.

---

### 3.3 Руководство пользователя

#### 3.3.1 Системные требования и развёртывание

Для развёртывания приложения требуется сервер под управлением ОС Linux (Ubuntu 22.04 LTS и выше), macOS или Windows с установленными Docker Engine 24+ и Docker Compose v2+. Все зависимости (JDK 17, PostgreSQL 16, Node.js) включены в Docker-образы и не требуют отдельной установки.

Последовательность запуска системы:
1. Клонировать репозиторий командой `git clone`;
2. Создать файл `.env` на основе `.env.example` и задать переменные окружения: строку подключения к БД, JWT-секрет, ключи API (YooKassa, Яндекс Метрика, OAuth2-провайдеры);
3. Выполнить команду `docker compose up --build` в корневой директории проекта;
4. После запуска приложение доступно по адресу `http://localhost:3000`; Swagger UI — по адресу `http://localhost:8080/swagger-ui.html`.

Схема взаимодействия контейнеров и внешних сервисов при развёртывании посредством Docker Compose представлена на рисунке 3.1.

> Рисунок 3.1 — Схема развёртывания приложения посредством Docker Compose
>
> *Рекомендации по построению:* UML Deployment Diagram или draw.io; три контейнера (db: 5432, backend: 8080, frontend: 3000) с указанием портов и сетевого взаимодействия внутри docker-сети. Внешние сервисы (YooKassa, Яндекс Метрика, SMTP) отображаются вне контура сети.*

#### 3.3.2 Сценарий 1: поиск и запись на мероприятие (роль — Житель)

1. Открыть главную страницу приложения и перейти в раздел «Афиша».
2. Применить фильтры по категории мероприятий (например, «Концерты») и диапазону дат.
3. Выбрать интересующее мероприятие и ознакомиться с его карточкой: описание, артисты, расписание сеансов, расположение на карте.
4. Нажать кнопку «Записаться» у выбранного сеанса.
   - *Бесплатное мероприятие:* подтвердить запись в модальном окне → QR-билет поступит на email.
   - *Платное мероприятие:* выбрать тип и количество билетов → перейти к оплате через YooKassa → QR-билет поступит на email после успешной оплаты.
5. Просмотреть билет в разделе личного кабинета «Мои билеты».

#### 3.3.3 Сценарий 2: публикация мероприятия (роль — Организатор)

1. Войти в систему и перейти в «Кабинет организатора» → «Создать мероприятие».
2. Заполнить мастер из шести шагов:
   - **Шаг 1:** название, краткое и полное описание, категории, возрастное ограничение;
   - **Шаг 2:** обложка и галерея фотографий;
   - **Шаг 3:** участвующие артисты;
   - **Шаг 4:** сеансы (дата, время, площадка, адрес);
   - **Шаг 5:** типы билетов и лимит мест;
   - **Шаг 6:** предпросмотр карточки и отправка на модерацию.
3. Дождаться решения администратора (уведомление поступит на email). После одобрения мероприятие автоматически публикуется на афише.
4. В разделе «Аналитика» отслеживать динамику регистраций, средний рейтинг и список участников.

#### 3.3.4 Сценарий 3: модерация и управление системой (роль — Администратор)

1. Войти в систему и перейти в «Панель администратора» → «Обзор системы»: просмотреть KPI-карточки и графики трафика (Яндекс Метрика).
2. Перейти в раздел «Мероприятия» → выбрать заявку со статусом «На рассмотрении».
3. Изучить карточку мероприятия и принять решение: одобрить или отклонить с указанием причины.
4. При необходимости перейти в раздел «Пользователи» и назначить / отозвать роль.
5. В разделе «Справочники» добавить новую площадку или категорию мероприятий.
...
**Список ожидания на сеансы.** При превышении лимита мест пользователи должны иметь возможность встать в очередь с автоматическим получением места при отмене чьей-либо регистрации. Таблица `session_waitlist` (позиция 32 в таблице 2.1) уже включена в схему базы данных.

**Push-уведомления.** Реализация Web Push API позволит доставлять уведомления о новых мероприятиях и приближающихся сеансах непосредственно в браузер пользователя. Таблица `notifications` (позиция 33) предусмотрена для хранения очереди уведомлений.

**Персонализированные рекомендации.** Таблица `user_interests` (позиция 31) заложена для хранения предпочтений пользователей по категориям. На её основе возможна реализация алгоритма рекомендаций, учитывающего индивидуальные интересы наряду с агрегированной популярностью событий.

**Промокоды.** Таблица `promo_codes` (позиция 34) предусмотрена для системы скидочных промокодов на платные мероприятия, актуальной при проведении акционных продаж билетов.

**Мобильное приложение.** Разработка нативных приложений для iOS и Android на базе существующего REST API обеспечит удобный доступ для большинства пользователей, предпочитающих мобильные устройства веб-браузеру.

**Масштабирование на несколько городов.** Архитектура системы изначально спроектирована с поддержкой справочника городов (таблица `cities`), что позволяет подключить новые населённые пункты через административный интерфейс без изменения кода.

---

### 3.5 Выводы по третьей главе

В третьей главе рассмотрены практические аспекты подготовки системы к эксплуатации.

Функциональное тестирование в объёме 16 тест-кейсов, охватывающих сценарии всех трёх ролей (житель, организатор, администратор), подтвердило корректность реализации всех заявленных функциональных требований: все тест-кейсы успешно пройдены.

Проверка безопасности по методологии OWASP Top 10 [48] не выявила критических уязвимостей. Реализованные меры — BCrypt, stateless JWT, параметризованные запросы через JPA, ролевое разграничение и Soft Delete — обеспечивают соответствие требованиям Федерального закона № 152-ФЗ «О персональных данных».

Нагрузочное тестирование (500 виртуальных пользователей, Apache JMeter) показало среднее время отклика 1 320 мс при доле ошибок 0,4%, что укладывается в нефункциональный норматив не более 2 с (таблица 1.8). Lighthouse-аудит подтвердил соответствие WCAG 2.1 уровня AA по всем ключевым страницам.

Разработано руководство пользователя с описанием порядка развёртывания системы и трёх ключевых пользовательских сценариев. Сформированы рекомендации по дальнейшему развитию в шести направлениях, опирающиеся на уже заложенную в архитектуре и схеме базы данных инфраструктуру (таблицы `session_waitlist`, `notifications`, `user_interests`, `promo_codes`).
...
4. Сформированы функциональные требования по восьми модулям (аутентификация, справочники, управление мероприятиями, афиша и поиск, регистрация, уведомления, отзывы, администрирование) и нефункциональные требования в восьми категориях.

5. Спроектирована клиент-серверная SPA-архитектура; технологический стек обоснован сравнительным анализом альтернатив: Java 17 / Spring Boot 3, React 18 / TypeScript, PostgreSQL 16, с интеграцией пяти внешних сервисов (YooKassa, Яндекс Карты, Яндекс Метрика, Яндекс Почта, OAuth2).

6. Спроектирована база данных из 35 таблиц с поддержкой Soft Delete, управлением схемой через Flyway и полным жизненным циклом мероприятия от создания черновика до архивирования.

7. Разработана серверная часть: REST API (12 групп эндпоинтов), аутентификация JWT + OAuth2 (Яндекс, Google), билетная система с QR-токенами, интеграция с платёжным шлюзом YooKassa и SMTP-сервером.

8. Спроектирован пользовательский интерфейс для трёх ролевых пространств с соблюдением принципов адаптивного дизайна и стандарта WCAG 2.1 уровня AA; бизнес-процесс публикации мероприятия формализован в виде BPMN-диаграммы.

9. Разработана клиентская часть (React 18, TypeScript, Vite): унифицированный HTTP-клиент, сервисный слой по паттерну «Фасад», защищённые маршруты для ролевого разграничения, интеграция с Яндекс Картами и Яндекс Метрикой.

10. Проведено тестирование: 16 тест-кейсов функционального тестирования пройдены успешно; нагрузочное тестирование (500 пользователей) подтвердило соответствие нормативу производительности; проверка по OWASP Top 10 не выявила критических уязвимостей.
